...
static void display_help(t_window *win_info) {
  ...
  win_info->leyend_img =
      mlx_texture_to_image(win_info->window, win_info->leyend_texture);
...
void set_window_title(char *file_name, t_window *win_info) {
  char *map_name = ft_strrchr(file_name, '/');
  if (!map_name)
    map_name = file_name;
  char *full_name = ft_calloc(1, 6 + ft_strlen(map_name) + 1);
...
void init_window_components(char *argv, t_window *win_info) {
  ...
  win_info->img =
      mlx_new_image(win_info->window, round(win_info->init_width * 0.95),
                    round(win_info->init_heigth * 0.95));
  mlx_image_to_window(win_info->window, win_info->img,
                      win_info->init_width * 0.025,
                      win_info->init_heigth * 0.005);
  ...
  if (win_info->map_dir)
    mlx_put_string(win_info->window, "Map_dir FOUND | Press TAB to swap map",
